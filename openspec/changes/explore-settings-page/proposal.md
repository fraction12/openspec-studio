...
- Group settings by scope: app-wide behavior, current repository continuity, local data/privacy, and feature-specific integrations when those features exist.
...
- Avoid dormant or speculative controls. Runner, provider, graph, timeline, or guided-workflow settings belong in Settings only after those capabilities exist and expose real configuration.
...
- Persistence model additions for user-controlled app behavior, plus helper operations for clearing recent repositories, repo UI state, and validation snapshots.
...
- Coordination with active changes:
  - `introduce-studio-runner` may later contribute runner endpoint/lifecycle controls to Settings, but this change should not invent runner controls before that feature exists.
