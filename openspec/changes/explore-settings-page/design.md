...
- runner endpoint, secret, lifecycle, or dispatch controls unless delivered by `introduce-studio-runner`;
...
- **Integrations**: render only settings contributed by implemented capabilities such as Studio Runner. No placeholder cards.
...
`introduce-studio-runner` owns runner configuration, signing secret handling, lifecycle controls, and dispatch history. Settings can host those controls after that change implements real runner behavior, but `app-settings` should provide only the general settings surface and extension location.
