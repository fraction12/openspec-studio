...
- Move current OpenSpec-specific behavior behind a built-in `openspec` provider adapter while preserving OpenSpec Studio's current product behavior.
...
- Future custom adapter/Foundry work can build on this contract without changing the core UI again.
